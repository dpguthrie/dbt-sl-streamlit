...
with tab2:
    ...
    if st.button("Submit Query", key="explore_submit_2"):
        if len(metrics) == 0:
            ...
            st.stop()
        with st.spinner(
            f'Fetching dimensions for the following metrics: **{", ".join(metrics)}**'
        ):
...
with tab3:
    ...
    all_dimensions = [
        v["dimensions"] for k, v in st.session_state.metric_dict.items() if k in metrics
    ]
    ...
    _tabbed_queries(
        "dimension_values",
        format={"metrics": metrics, "dimension": dimensions},
        variables={"metrics": metrics, "groupBy": dimensions},
    )
